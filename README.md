...
```mermaid
flowchart LR
    U[Operator or Supervisor] --> F[React Frontend]
    F --> MAP[Azure Maps UI]
    F --> AUTH[Azure AD via MSAL]
    F --> API[FastAPI API]

    API --> ORCH[Orchestrator Service]
    ORCH --> TRIAGE[TriageAgent]
    ORCH --> DISPATCH[DispatchAgent]
    ORCH --> STRATEGY[ResponseStrategyAgent]
    ORCH --> TRANSPORT[TransportRiskAgent]
    ORCH --> TRACE[Decision Trace Service]
    ORCH --> ROUTING[Routing and Dispatch Engine]

    TRIAGE --> RAG[RAG Service]
    RAG --> SEARCH[Azure AI Search]

    ROUTING --> MAPSVC[Azure Maps Services]
    API --> DB[(MongoDB or Cosmos DB)]
...
sequenceDiagram
    ...
    Ops->>UI: Monitor or select incident
    UI->>API: Request operational data
    API->>Orch: Trigger orchestration
    Orch->>T: Analyze severity with retrieved context
    Orch->>D: Select best ambulance and clinic
    Orch->>TR: Evaluate transport risk
    Orch->>DB: Persist decisions and traces
    API-->>UI: Return analytics, routing, and trace data
```
